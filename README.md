...
| 6 | إدارة الطلبات (Dashboard) | ✅ مكتمل |
...
| 12 | التقارير والإحصائيات المتقدمة | ✅ مكتمل |
| 13-15 | إشعارات + ثيمات + إعدادات متقدمة | 🔜 التالي |
...
### محادثة ١٢ — التقارير والإحصائيات المتقدمة 📊 (فبراير ٢٠٢٦)

| الصفحة | المسار | الوصف |
|-------|-------|------|
| التقارير | `/dashboard/reports` | لوحة تحليلات شاملة بـ ٥ تبويبات |

- ✅ تبويب نظرة عامة: ٤ بطاقات KPI (إيرادات + طلبات + متوسط + عملاء) مع نسب نمو
- ✅ تبويب المبيعات: رسم بياني يومي/أسبوعي/شهري + جدول تفصيلي
- ✅ تبويب المنتجات: ترتيب أفضل ١٠ منتجات (إيرادات/كمية/طلبات) + أداء التصنيفات
- ✅ تبويب الطلبات: تحليل حسب الحالة/الدفع/الشحن + توزيع ساعي + معدل تحويل
- ✅ تبويب العملاء: إحصائيات + أفضل عملاء + رسم اكتساب يومي
- ✅ فلترة حسب الفترة (٧ أيام / ٣٠ يوم / ٩٠ يوم / سنة) مع مقارنة نمو
- ✅ مكونات CSS-only: Mini Bar Chart + Progress Ring + Horizontal Bars
- ✅ Zustand Reports Store
- ✅ تصميم responsive (mobile + desktop)
- ✅ Tooltips تفاعلية على الرسوم البيانية
...
- ✅ قائمة المفضلة (Wishlist) — إضافة/إزالة + صفحة كاملة
- ✅ Zustand Wishlist Store
- ✅ أيقونة المفضلة + عداد في Header
- ✅ Mobile Bottom Navigation Bar (رئيسية / منتجات / مفضلة / حسابي)
- ✅ نظام تقييمات المنتجات (نجوم + فورم تقييم + ملخص إحصائي)
- ✅ زر المفضلة على صفحة المنتج
- ✅ صفحات ثابتة (عرض: من نحن، الشروط...)
- ✅ Footer محدث مع روابط الصفحات الثابتة
- ✅ لوحة تحكم: إدارة التقييمات (نشر/رفض/حذف + إحصائيات + فلاتر)
- ✅ لوحة تحكم: إدارة الصفحات (CRUD كامل)
- ✅ Sidebar محدث (إضافة التقييمات + الصفحات)

### المحادثة ٩ — المالية + الشحن + Checkout ديناميكي 💰 (فبراير ٢٠٢٦)

| الصفحة | المسار | الوصف |
|-------|-------|------|
| المالية | `/dashboard/payments` | إيرادات + مدفوعات + تقسيم حسب الدفع + معاملات |
| الشحن | `/dashboard/shipping` | إحصائيات شحن + قائمة شحنات + فلتر |

- ✅ ٤ بطاقات مالية (إيرادات، محصّل، معلّق، متوسط طلب)
- ✅ مقارنة فترات (اليوم، الشهر، الشهر الماضي + نمو%)
- ✅ تقسيم حسب طريقة الدفع (progress bars)
- ✅ جدول معاملات (فلتر: الكل/مدفوع/معلّق) + pagination
- ✅ إحصائيات الشحن + توزيع + قائمة شحنات
- ✅ Checkout ديناميكي (طرق دفع/شحن من إعدادات التاجر)
- ✅ شحن مجاني تلقائي (بانر أخضر/أصفر)
- ✅ بيانات التحويل البنكي في Checkout

### المحادثة ٨ — الكوبونات + الإعدادات 🎟️ (فبراير ٢٠٢٦)

| الصفحة | المسار | الوصف |
|-------|-------|------|
| إدارة الكوبونات | `/dashboard/marketing` | إحصائيات + بحث + فلتر + تفعيل/حذف |
| إنشاء كوبون | `/dashboard/marketing/coupons/new` | نوع + قيمة + شروط + فترة + معاينة |
| تعديل كوبون | `/dashboard/marketing/coupons/[id]/edit` | تعديل + سجل استخدام |
| إعدادات المتجر | `/dashboard/settings` | ٦ تبويبات (متجر/دفع/شحن/طلب/تواصل/إشعارات) |

- ✅ ٤ بطاقات إحصائية (إجمالي كوبونات، نشط، استخدام، خصومات)
- ✅ فلتر حالة (نشط/منتهي/مستنفد/معطل) + بحث بالكود
- ✅ إنشاء كوبون (نسبة/مبلغ ثابت + كود عشوائي + شروط + فترة صلاحية)
- ✅ تعديل كوبون + عرض سجل الاستخدام (طلبات)
- ✅ تفعيل/تعطيل + حذف (modal تأكيد)
- ✅ إعدادات: المتجر (اسم/وصف/عملة/لوقو)
- ✅ إعدادات: الدفع (COD/تحويل بنكي/SkyPay/سداد)
- ✅ إعدادات: الشحن (Aramex/DHL/استلام/شحن مجاني)
- ✅ إعدادات: الطلب + التواصل + الإشعارات
- ✅ Toggle cards + Toast notifications
- ✅ Zustand Coupons Store + Settings Store

### المحادثة ٧ — إدارة العملاء + حساب المستهلك 👥 (فبراير ٢٠٢٦)

| الصفحة | المسار | الوصف |
|-------|-------|------|
| قائمة العملاء | `/dashboard/customers` | إحصائيات + بحث + ترتيب |
| تفاصيل العميل | `/dashboard/customers/[id]` | طلبات + عناوين + ملاحظات + حظر |
| حساب المستهلك | `/store/[store]/account` | دخول OTP + طلباتي + عناويني + بياناتي |

- ✅ بطاقات إحصائية (إجمالي العملاء، جدد اليوم، إيرادات)
- ✅ بحث (اسم/جوال/إيميل) + ترتيب (أحدث/طلبات/إنفاق/اسم)
- ✅ تفاصيل العميل: إحصائيات + سجل طلبات + عناوين + ملاحظات + حظر
- ✅ حساب المستهلك: OTP login + tabs (طلباتي/عناويني/بياناتي)
- ✅ رابط "حسابي" + أيقونة في header المتجر
- ✅ Zustand Customers Store

### المحادثة ٦ — إدارة الطلبات 📋 (فبراير ٢٠٢٦)

| الصفحة | المسار | الوصف |
|-------|-------|------|
| قائمة الطلبات | `/dashboard/orders` | إحصائيات + بحث + فلاتر + جدول |
| تفاصيل الطلب | `/dashboard/orders/[id]` | منتجات + حالة + timeline + فاتورة |

- ✅ بطاقات إحصائية (طلبات جديدة، معالجة، اليوم، إيرادات)
- ✅ بحث (رقم طلب / اسم عميل / جوال) + فلتر حالة (pills) + ترتيب
- ✅ جدول Desktop + بطاقات Mobile + pagination
- ✅ تفاصيل الطلب: منتجات + ملخص مالي + عنوان + عميل
- ✅ تحديث حالة الطلب (modal + flow: جديد→مؤكد→تجهيز→شحن→توصيل)
- ✅ Timeline تاريخ حالات الطلب
- ✅ ملاحظات التاجر (حفظ) + تأكيد الدفع يدوياً
- ✅ طباعة الفاتورة (print styles)
- ✅ Zustand Orders Store (fetch, filter, update, stats)
