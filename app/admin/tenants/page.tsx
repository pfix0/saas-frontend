...
/**
 * ساس — Platform Admin: Tenants (Stores) Management
 */
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="بحث بالاسم، الرابط، البريد..."
      ...
      <div className="bg-grey-800/50 rounded-xl border border-grey-700/30 overflow-hidden">
        {isLoading ? (
          <div className="p-6 space-y-3">
            {[1, 2, 3, 4].map((i) => <div key={i} className="h-16 bg-grey-700/30 rounded-lg animate-pulse" />)}
          </div>
        ) : tenants.length === 0 ? (
                ...
                {tenants.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-grey-700/20 hover:bg-grey-700/20 transition-colors group">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[0.65rem] font-bold text-white ${plan.color}`}>
                          {plan.label}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[0.65rem] font-bold border ${status.class}`}>
                          {status.label}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="relative">
                          <button
                            onClick={() => setActionTenant(actionTenant?.id === t.id ? null : t)}
                          ...
                          {actionTenant?.id === t.id && (
                            <div className="absolute left-0 top-full mt-1 w-48 bg-grey-700 rounded-xl border border-grey-600 shadow-lg z-20 overflow-hidden animate-fade-in">
                              {/* Status */}
                                  ...
                                  {(['active', 'trial', 'suspended'] as const).map((s) => (
                                    <button
                                      ...
                                      className={`text-[0.6rem] px-2 py-0.5 rounded-full border ${
                                        t.status === s
                                          ? statusOptions[s].class
                                          : 'text-grey-500 border-grey-600 hover:border-grey-500'
                                      } transition-all`}
                                    >
                              ...
                              <div className="px-3 py-2 border-b border-grey-600">
                                ...
                                <div className="flex gap-1">
                                  {(['basic', 'growth', 'pro'] as const).map((p) => (
                                    <button
                                      key={p}
                                      onClick={() => handlePlanChange(t.id, p)}
                                      className={`text-[0.6rem] px-2 py-0.5 rounded-full ${
                                        t.plan === p
                                          ? `${planLabels[p].color} text-white`
                                          : 'bg-grey-600 text-grey-400 hover:bg-grey-500'
                                      } transition-all`}
                                    >
                                      {planLabels[p].label}
                                    </button>
                              ...
                              {/* Delete */}
                              <button
                                onClick={() => { setDeleteId(t.id); setActionTenant(null); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-grey-800 rounded-2xl p-6 w-full max-w-sm border border-grey-700 animate-fade-in">
            ...
            <p className="text-sm text-grey-400 text-center mb-6">
              سيتم حذف المتجر وكل بياناته (المنتجات، الطلبات، العملاء). هذا الإجراء لا يمكن التراجع عنه.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="flex-1 py-2.5 rounded-xl bg-grey-700 text-grey-300 text-sm font-semibold hover:bg-grey-600 transition-all">
                إلغاء
              </button>
              <button onClick={handleDelete} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-500 transition-all flex items-center justify-center gap-1">
                <span className="material-icons-outlined text-sm">delete</span>
                حذف
              </button>
            </div>
